package no.uis.security.dsa.service;

import no.uis.security.common.service.BasicService;
import no.uis.security.dsa.model.GlobalPublicKey;
import no.uis.security.dsa.model.User;
import no.uis.security.dsa.model.UserKeys;

/**
 * Created with IntelliJ IDEA.
 * User: maziarkaveh
 * Date: 04.11.13
 * Time: 14:47
 * To change this template use File | Settings | File Templates.
 */
public interface UserKeysService extends BasicService<UserKeys,Long> {
    UserKeys retrieveOrGenerateAndAssignUserKeys(User user,GlobalPublicKey globalPublicKey);
}
